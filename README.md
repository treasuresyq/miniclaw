<p align="center">
  <img src="web/public/icons/icon-512.png" alt="Miniclaw logo" width="96" /><br />
...
- [🖥 界面与体验](#-界面与体验)
- [🎯 核心功能](#-核心功能)
  - [Agent 与 Workspace](#agent-与-workspace)
  - [Pi Agent Runtime](#pi-agent-runtime)
  - [Memory 与上下文](#memory-与上下文)
  - [Skills、MCP 与 Plugins](#skillsmcp-与-plugins)
  - [渠道与自动化](#渠道与自动化)
  - [Electron Desktop](#electron-desktop)
- [🏗 工作原理](#-工作原理)
- [🚀 快速开始](#-快速开始)
  - [环境要求](#环境要求)
  - [启动 Backend 与 Web Client](#启动-backend-与-web-client)
  - [启动 Electron Desktop](#启动-electron-desktop)
- [🧰 常用命令](#-常用命令)
- [⚙️ 配置入口](#️-配置入口)
- [🔒 执行与安全边界](#-执行与安全边界)
- [📊 项目状态](#-项目状态)
...
Miniclaw 是一个面向个人与团队的自托管 AI Agent 工作台。它不是一个只保存聊天记录的对话框，而是把以下对象放在**同一个产品模型**中：
...
> Miniclaw 采用**本地优先 + 显式集成**的思路：数据库、工作区元数据、会话状态和配置由自己的服务管理；模型、消息渠道、Docker 与外部工具都作为**可配置边界**接入。
...
<p align="center"><em>工作台：在同一个窗口中管理 Agent、Workspace、Session 与对话。</em></p>
...
<p align="center">
  <img src="web/public/icons/icon-512.png" alt="Miniclaw application icon" width="160" />
</p>
...
- Runtime 不会静默假装拥有缺失能力；尚未接入 Pi 的 Web Search/Web Fetch 能力会保持**明确不可用**。

### Memory 与上下文

- **Memory 按 Workspace 隔离**，不在不同工作区之间隐式共享。
- 支持**事实、偏好、决策和经验**等知识类型，以及搜索、编辑、忘记和版本历史。
- 写入使用 **revision 与 compare-and-set**，避免并发编辑覆盖彼此的修改。
- 每条 Memory 可记录**来源 Session、来源类型、观察时间和变更历史**，便于回溯 provenance。
...
- Agent 运行前会计算**有效能力 Manifest**，并把 Skills、MCP、Memory、Workspace 与渠道上下文按策略注入。
- 路径穿越、符号链接逃逸、越权 Workspace 与未授权 capability 会在边界层拒绝。

### 渠道与自动化

- 支持将消息渠道绑定到指定 Workspace 或 Session，并按**用户、群聊、话题和 owner 规则**进行 ACL 判断。
- 支持 **飞书、Telegram、QQ、钉钉、微信、Discord、WhatsApp** 等消息入口。
...
```bash
git clone https://github.com/treasuresqi/miniclaw.git
cd miniclaw
...
> ⚠️ **不要把 API Key、Session Cookie 或其他凭证写入命令行历史、截图或提交到仓库。** 远程部署时使用 HTTPS/WSS，并为反向代理、Cookie 和访问控制配置独立的安全边界。
...
<p align="center">
  <sub>Made with ❤️ for the Agent community · Powered by Pi Agent Runtime</sub>
</p>
